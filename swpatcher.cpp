...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < argc; ++i) {
        ...
        if(arg.substr(0, 2) != "--") help(-1, "error: invalid argument format: missing `--`");
        arg.assign(arg.data()+2);
        size_t eqpos = arg.find('=');
        if(eqpos == std::string::npos) help(-1, "error: invalid argument format: missing `=` after argument name or `--`\n");
        std::string argName = arg.substr(0, eqpos);
        if(argName.size() == 0) help(-1, "error: invalid argument format: argument name is empty\n");
        if(std::find(std::begin(argNames), std::end(argNames), argName) == std::end(argNames)) help(-1, "Invalid argument `%s`\n", argName.c_str());
    ...
    if(workshopPath.empty()) {
        workshopPath = std::filesystem::current_path().parent_path().parent_path().string() + "/workshop/content";
    }
    auto originalPath = workshopPath + "/" + std::to_string(appid);
    auto patchedPath = originalPath + suffix;

    fs::copy(patchedPath, originalPath, fs::copy_options::overwrite_existing | fs::copy_options::recursive);
